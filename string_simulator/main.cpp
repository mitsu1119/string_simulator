...
DWORD ReadWave(LPDIRECTSOUNDBUFFER pDSBuffer, DWORD dwSize) {
	...
	if(read_pos == BUF_SEC * BUF_DIVIDES * 4) {
		read_pos++;
		return 1;
	}
...
int initDs(HWND hWnd) {
	...
	dsbd2.dwBufferBytes = wfx2.nAvgBytesPerSec >> 0;	// 1秒分のセカンダリバッファ
	...
	// 長くてもBUF_SEC秒分
	pulses_len = 4 * dwBufferSize * BUF_SEC;
